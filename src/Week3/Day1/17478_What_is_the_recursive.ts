  ...
  fs
    .readFileSync(
      process.platform === 'linux'
        ? '/dev/stdin'
        : `${PWD}/src/Week3/Day1/17478_What_is_the_recursive.txt`
    )
    .toString()
    .trim()
)

const recursive = (count: number, current: number) => {
  const underBar = Array.from({ length: current * 4 })
    .map(() => '_')
    .join('')
